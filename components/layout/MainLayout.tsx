...
export default function MainLayout({children}: MainLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <header
        className={`bg-white shadow-sm fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'shadow-md' : ''}`}>
      ...
      <div
        className="hidden md:block fixed left-0 top-16 h-screen w-64 bg-white shadow-sm border-r border-gray-200 z-40 transition-all duration-300 overflow-y-auto">
        <div className="p-4">
          {/* User Profile Card */}
          {isAuthenticated ? (
            <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
              <div className="flex items-center mb-3">
                <div
                  className="h-12 w-12 rounded-full bg-gray-100 flex items-center justify-center text-[#008247] text-xl font-bold mr-3">
                  {user?.username?.charAt(0).toUpperCase() || 'U'}
                </div>
                <div>
                ...
                {isAuthenticated ? (
                  <>
                    ...
                    <li><Link href={`/profile/${user?.id}`}
                              className="text-gray-600 hover:text-[#008247]">Profile</Link></li>
